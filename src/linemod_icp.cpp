...
float matToVec(const cv::Mat_<cv::Vec3f> &src_, std::vector<cv::Vec3f> &pts_, const cv::Mat_<uchar>& mask)
{
  assert(src_.size()==mask.size());
  pts_.clear();
  cv::MatConstIterator_<cv::Vec3f> it_src = src_.begin();
  cv::MatConstIterator_<uchar> it_mask = mask.begin();
  int cnt=0;
  for(;it_src!=src_.end();++it_src,++it_mask)
  {
    if(!*it_mask || !cv::checkRange(*it_src))
      continue;
    pts_.push_back(*it_src);
    cnt++;
  }
  return static_cast<float>(cnt)/static_cast<float>(src_.size().area());
}
